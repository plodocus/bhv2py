...
def read_bhv(filename):
    ...
    with open(filename, "rb") as f:
        ...
        if bhvver > 2.65:
            ...
            info_by_cond = [
                info_by_cond[(i * 128) : (i + 1) * 128].split()
                for i in range(bhv["NumConds"])
            ]
            ibc = []
        ...
        if bhvver > 2.5:
            ...
            for mov in MOV:
                ...
                for fr in range(nframes):
                    data[:, :, :, fr] = r.read(f, "uint8", np.prod(sz)).reshape(
                        (sz), order="F"
                    )
                mov["Data"] = data
            bhv["Stimuli"]["MOV"] = MOV

        r.read(f, "uint8", 1024)  # unused padding
                    ...
                    for i in range(numstat):
                        ...
                        if np.any(np.array(status[-1]) > 1):
                            ...
                            for fnum in range(numfields):
                                ...
                                print(datacount)
                                data_this.append(
                                    [r.read(f, "double") for i in range(datacount)]
                                )
